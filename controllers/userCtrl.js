...
const applyFitnessConsultant = async (req, res) => {
    try {
        const newFitnessConsultant = await fitnessConsultantModel({...req.body, status:'pending'})
        await newFitnessConsultant.save()
        const adiminUser = await userModel.findOne({isAdmin:true})
        ...
        notification.push({
            ...
            data:{
                ...
                name:newFitnessConsultant.firstName + ' ' + newFitnessConsultant.lastName,
...
const getAllFitnessConsultantsController = async (req, res) => {
    try {
        const fitnessConsultants = await fitnessConsultantModel.find({status:'approved'});
        res.status(200).send({
            success:true,
...
const bookAppointmentController = async (req, res) => {
    try {
        ...
        const user = await userModel.findOne({_id: req.body.fitnessConsultantInfo.userId});
...
const bookAvailabilityController = async (req, res) => {
    try {
        ...
        const appointments = await appointmentModel.find({fitnessConsultantId,
            date,
            time:{
